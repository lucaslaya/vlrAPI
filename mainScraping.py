...
#|_|  |_|\__,_|\__\___|_| |_|
    ...
    def vlr_match(self, match_id=66842):
        ...
        game_maps = games_container.find_all("div", {"class": "vm-stats-gamesnav-item"})
        ...
        url = f'https://www.vlr.gg/{match_id}/?tab=overview'
        soup, status = getSoup(url)

        stats_container = soup.find("div", {"class": "vm-stats-container"})

        for i in range(len(game)):
            stats = stats_container.find("div", {"data-game-id": game[i]})

            game_map = game_names[i]
            
            team_stats_container = stats.find_all("table", {"class": "wf-table-inset"})

            # Team 1 
            team1_stats_body = team_stats_container[0].find("tbody")
            team1_player = team1_stats_body.find_all("tr")

            for module in team1_player:
...
#       |_|                                |___/ 
    ...
    def vlr_upcoming(self, page=1):
        ...
        #    outfile.write(json_string, sort_keys=False, indent=4)
